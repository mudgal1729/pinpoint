...
## The demo
One hardcoded case, both calls in Hindi: a man in his 50s sends laddoo to his sister in Jaipur; the address has Pannadhay Circle and a house number, but the pin sits about 2 km away in Pratapnagar. A dashboard previews the orchestrator (the order, the issue, the agent's goals, and the status), with a button to trigger each call. Agent 1 confirms the address and the order edit (Haldiram substitute with a 20 rupee refund, or two 500 g packs if asked); the status then shows the recipient being called; Agent 2 confirms the route and protects the sender identity. We build the dashboard and the two calls; everything else (detection, geocoding, real fulfillment, telephony) is described, not built. The time goes into making the conversations smooth.
...
- **Presentation and usability:** a smooth, natural live voice call in Hindi, with the orchestrator's state legible on screen and a graceful failure path.
